...
# ========== 1. 添加依赖（ExoPlayer + OkHttp + Gson + Preference） ==========
...
# ========== 3. 修改 AndroidManifest.xml（安全删除并添加 LAUNCHER） ==========
echo "🛠️ 修改 AndroidManifest.xml..."

# 删除所有包含 LAUNCHER 的行（包括 action.MAIN 和 category.LAUNCHER）
sed -i '/android.intent.action.MAIN/d' "$MANIFEST"
sed -i '/android.intent.category.LAUNCHER/d' "$MANIFEST"

# 在第一个 <activity> 前插入新的 LAUNCHER 声明
# 注意：使用 awk 更安全，但这里用 sed 插入到第一个 activity 之前
sed -i "0,/^[[:space:]]*<activity/s//    <activity android:name=\"$PKG.$MAIN_ACT_SIMPLE\" android:exported=\"true\">\n        <intent-filter>\n            <action android:name=\"android.intent.action.MAIN\" \/>\n            <category android:name=\"android.intent.category.LAUNCHER\" \/>\n        <\/intent-filter>\n    <\/activity>\n\n    &/" "$MANIFEST"
